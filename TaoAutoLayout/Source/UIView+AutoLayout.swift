...
public enum tao_AlignType {
    ...
    private func layoutAttributes(isInner: Bool, isVertical: Bool) -> tao_LayoutAttributes {
        let attributes = tao_LayoutAttributes()
        ...
        case .CenterCenter:     // 仅内部参照需要
            return tao_LayoutAttributes(horizontal: .CenterX, referHorizontal: .CenterX, vertical: .CenterY, referVertical: .CenterY)
        }
    }
}
...
extension UIView {
    ...
    public func tao_AlignInner(type type: tao_AlignType, referView: UIView, size: CGSize?, offset: CGPoint = CGPointZero) -> [NSLayoutConstraint]  {
        ...
        return tao_AlignLayout(referView, attributes: type.layoutAttributes(true, isVertical: true), size: size, offset: offset)
    }
    ...
    public func tao_AlignVertical(type type: tao_AlignType, referView: UIView, size: CGSize?, offset: CGPoint = CGPointZero) -> [NSLayoutConstraint] {
        ...
        return tao_AlignLayout(referView, attributes: type.layoutAttributes(false, isVertical: true), size: size, offset: offset)
    }
    ...
    public func tao_AlignHorizontal(type type: tao_AlignType, referView: UIView, size: CGSize?, offset: CGPoint = CGPointZero) -> [NSLayoutConstraint] {
        ...
        return tao_AlignLayout(referView, attributes: type.layoutAttributes(false, isVertical: false), size: size, offset: offset)
    }
    ...
    public func tao_HorizontalTile(views: [UIView], insets: UIEdgeInsets) -> [NSLayoutConstraint] {
        ...
        firstView.tao_AlignInner(type: tao_AlignType.TopLeft, referView: self, size: nil, offset: CGPoint(x: insets.left, y: insets.top))
        ...
        for i in 1..<views.count {
            let subView = views[i]
            cons += subView.tao_sizeConstraints(firstView)
            subView.tao_AlignHorizontal(type: tao_AlignType.TopRight, referView: preView, size: nil, offset: CGPoint(x: insets.right, y: 0))
            preView = subView
        }
    ...
    public func tao_VerticalTile(views: [UIView], insets: UIEdgeInsets) -> [NSLayoutConstraint] {
        ...
        for i in 1..<views.count {
            let subView = views[i]
            cons += subView.tao_sizeConstraints(firstView)
            subView.tao_AlignVertical(type: tao_AlignType.BottomLeft, referView: preView, size: nil, offset: CGPoint(x: 0, y: insets.bottom))
            preView = subView
        }
    ...
    ///
    ///  - returns: 约束数组
    private func tao_AlignLayout(referView: UIView, attributes: tao_LayoutAttributes, size: CGSize?, offset: CGPoint) -> [NSLayoutConstraint] {
        
        translatesAutoresizingMaskIntoConstraints = false
        
        var cons = [NSLayoutConstraint]()
        
        cons += tao_positionConstraints(referView, attributes: attributes, offset: offset)
        
        if size != nil {
            cons += tao_sizeConstraints(size!)
        }
    ...
    ///
    ///  - returns: 约束数组
    private func tao_sizeConstraints(size: CGSize) -> [NSLayoutConstraint] {
    ...
    ///
    ///  - returns: 约束数组
    private func tao_sizeConstraints(referView: UIView) -> [NSLayoutConstraint] {
    ...
    ///
    ///  - returns: 约束数组
    private func tao_positionConstraints(referView: UIView, attributes: tao_LayoutAttributes, offset: CGPoint) -> [NSLayoutConstraint] {
...
///  布局属性
private final class tao_LayoutAttributes {
